...
class ProductManager {
  ...
  addProduct = async (title, description, price, thumbnail, stock,id) => {
    const newId = products.length ? products[ products.length - 1 ].id + 1 : 0
    const newProduct = new Product (title, description, price, thumbnail, stock, code, newId );
    this.products.push(newProduct);
    await this.fileSystem.promises.writeFile(this.path, JSON.stringify(this.products));
    try {
